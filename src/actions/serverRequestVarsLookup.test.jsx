...
describe('urlFromEnv : URL from environment variables', () => {
  ...
  it('Should successfully retrieve a full url from a process.env var', () => {
    ...
    const url = serverRequestVarsLookup('EVENTS_EXT')
  ...
  it('Should tell you if you have not defined the data server root', () => {
    process.env.EVENTS_EXT = '/api/curfew/export_events/deeprows'
    expect(() => serverRequestVarsLookup('EVENTS_EXT'))
  ...
  it('Should tell you if the env var is not present', () => {
    process.env.SERVER_ROOT = 'http://localhost:4040'
    expect(() => serverRequestVarsLookup('EVENTS_EXT')).toThrow(new Error(missingUrlErrorMessage))
  })
